...
var snake = {
  ...
  var x = [];
  var holder = [];
  x = (Object.getOwnPropertyNames(snake));
...
var secondHero = heroes.splice(3, 0);
...
function getValueOfProperty (obj, p){
	return obj[p];
}
...
function makeChatMessage (mes, author){
	var ob = {};
	ob.message = mes;
	ob.author = author;
...
function coderTest (x) {
	if (x.name == "Jeremy"){
		x.lovesCode = 10;
	}else if(x.name == "Brack"){
		x.lovesCode = 0;
	}else{
		x.lovesCode = 5;
	}
